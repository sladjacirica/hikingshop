 
<?php include "partials/header.php"; ?>

<!--header-->
<header>
    <article >
        <h1>Online outdoor shop,buy online outdoor clothing & trekking</h1>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla provident
             facilis maiores omnis quo est odit voluptas. Soluta, praesentium at?
             Minus aliquam magnam incidunt possimus itaque sunt aliquid, atque fugit!</p>
             <button>Order now</button>
    </article>
</header>

<!--featuredItems-->
<section class="featuredItems container">
    <h2>Featured items</h2>
    <article><div class="owl-carousel owl-theme">
        <a class="item"  href="boots.php">
        <img src="img/boots.jpg" alt="">
        <h3>Boots</h3>
        <p>$210</p>
        </a>
        <a class="item"  href="pants.php">
        <img src="img/pants.jpg" alt="">
        <h3>Pants</h3>
        <p>$40</p>
        </a>
        <a class="item"  href="boots.php">
        <img src="img/poles.jpg" alt="">
        <h3>Poles</h3>
        <p>$50</p>
        </a>
        <a class="item"  href="boots.php">
        <img src="img/jackets.jpg" alt="">
        <h3>Pants</h3>
        <p>$550</p>
        </a>
        <a class="item"  href="boots.php">
        <img src="img/skirts.jpg" alt="">
        <h3>Skirts</h3>
        <p>$60</p>
        </a>
        <a class="item"  href="boots.php">
        <img src="img/shirts.jpg" alt="">
        <h3>Shirts</h3>
        <p>$300</p>
        </a>
        <a class="item"  href="boots.php">
        <img src="img/socks.jpg" alt="">
        <h3>Socks</h3>
        <p>$10</p>
        </a>
        <a class="item"  href="boots.php">
        <img src="img/gloves.jpg" alt="">
        <h3>Gloves</h3>
        <p>$25</p>
        </a>
    </article>
</section>



<?php include "partials/footer.php"; ?>